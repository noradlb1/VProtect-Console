...
namespace VProtect.Core.OpCodes
{
    internal class Bge : IOpCode
    {
        public OpCode Code
        {
            get { return dnlib.DotNet.Emit.OpCodes.Bge; }
        }

        public void Run(VMContext ctx, dynamic operand)
        {
            ctx.Instructions.Add(new VInstruction(VCode.Bge, ctx.SelectedMethod.Body.Instructions.IndexOf((Instruction)operand)));
        }
    }
}