...
namespace VProtect.Core.OpCodes
{
    internal class Ldarg : IOpCode
    {
        public OpCode Code
        {
            get { return dnlib.DotNet.Emit.OpCodes.Ldarg; }
        }

        public void Run(VMContext ctx, dynamic operand)
        {
            var paramType = ((Parameter)operand).Type;
            ctx.Instructions.Add(new VInstruction(VCode.Ldarg, ctx.SelectedMethod.Parameters.IndexOf((Parameter)operand), MDTokenResolver.Resolve(ctx, paramType)));
        }
    }
}