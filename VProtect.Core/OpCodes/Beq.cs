...
namespace VProtect.Core.OpCodes
{
    internal class Beq : IOpCode
    {
        public OpCode Code
        {
            get { return dnlib.DotNet.Emit.OpCodes.Beq; }
        }

        public void Run(VMContext ctx, dynamic operand)
        {
            ctx.Instructions.Add(new VInstruction(VCode.Beq, ctx.SelectedMethod.Body.Instructions.IndexOf((Instruction)operand)));
        }
    }
}